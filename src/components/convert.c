...
typedef struct bl_conv_s2u_block_s {
    bl_pin_s32_t in;
	bl_pin_u32_t out;
} bl_conv_s2u_block_t;

_Static_assert((sizeof(bl_conv_s2u_block_t) < BL_BLOCK_DATA_MAX_SIZE), "block structure too large");


/* array of pin definitions - one copy in FLASH */
static bl_pin_def_t const bl_conv_s2u_pins[] = {
    { "in", BL_TYPE_S32, BL_DIR_IN, offsetof(bl_conv_s2u_block_t, in)},
    { "out", BL_TYPE_U32, BL_DIR_OUT, offsetof(bl_conv_s2u_block_t, out)}
};
...
static void bl_conv_s2u_function(void *ptr, uint32_t period_ns)
{
    ...
    bl_conv_s2u_block_t *p = (bl_conv_s2u_block_t *)ptr;
    int32_t in = *(p->in);
    if ( in > 0 ) {
        *(p->out) = (uint32_t)in;
    } else {
