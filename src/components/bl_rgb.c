...
typedef struct bl_rgb_instance_s {
    bl_rgb_config_t config;
    uint16_t padding;
} bl_rgb_instance_t;

_Static_assert( (sizeof(bl_rgb_instance_t) % 4) == 0);
...
#define RGB_MAX_INSTANCE_SIZE (sizeof(bl_rgb_instance_t)+RGB_MAX_CHANS*sizeof(rgb_chan_rt_data_t))

_Static_assert((RGB_MAX_PINS < BL_PIN_COUNT_MAX), "too many pins");
_Static_assert((RGB_MAX_INSTANCE_SIZE < BL_INSTANCE_DATA_MAX_SIZE), "instance structure too large");
...
struct bl_instance_meta_s *rgb_setup(char const *instance_name, struct bl_comp_def_s const *comp_def, void const *personality);

// component definitions - one copy in FLASH
bl_comp_def_t const bl_rgb_merge_def = {
    "rgb_merge",
    rgb_setup,
    sizeof(bl_rgb_instance_t),
...
struct bl_instance_meta_s *rgb_setup(char const *instance_name, struct bl_comp_def_s const *comp_def, void const *personality)
{
    ...
    // now the emblocs setup - create an instance of the proper size to include all the channel data
    meta = bl_instance_create(instance_name, comp_def, comp_def->data_size + (cfg_info->num_chan * sizeof(rgb_chan_rt_data_t)));
    CHECK_RETURN(meta);
    ...
    for ( int n = 0 ; n < cfg_info->num_chan ; n++ ) {
        ...
        pindef.data_offset = TO_INSTANCE_SIZE((uint32_t)((char *)&(chan_data->merged) - (char *)data));
        // create the pin
        ...
        pindef.data_offset = TO_INSTANCE_SIZE((uint32_t)((char *)&(chan_data->red) - (char *)data));
        result = bl_instance_add_pin(meta, &pindef);
        CHECK_RETURN(result);
        // next pin
        name = malloc(10);
        ...
        strcat(name, "_grn");
        pindef.name = name;
        pindef.data_offset = TO_INSTANCE_SIZE((uint32_t)((char *)&(chan_data->grn) - (char *)data));
        result = bl_instance_add_pin(meta, &pindef);
        CHECK_RETURN(result);
        // next pin
        name = malloc(10);
        ...
        strcat(name, "_blu");
        pindef.name = name;
        pindef.data_offset = TO_INSTANCE_SIZE((uint32_t)((char *)&(chan_data->blu) - (char *)data));
        result = bl_instance_add_pin(meta, &pindef);
        CHECK_RETURN(result);
        // next channel
        chan_data++;
