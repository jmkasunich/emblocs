...
typedef struct bl_perftimer_block_s {
    bl_pin_u32_t time;
	uint32_t tsc;
} bl_perftimer_block_t;

_Static_assert((sizeof(bl_perftimer_block_t) < BL_BLOCK_DATA_MAX_SIZE), "block structure too large");


// array of pin definitions - one copy in FLASH
static bl_pin_def_t const bl_perftimer_pins[] = {
    { "time", BL_TYPE_U32, BL_DIR_OUT, offsetof(bl_perftimer_block_t, time)}
};
...
struct bl_block_meta_s *bl_perftimer_setup (char const *block_name, struct bl_comp_def_s const *comp_def, void const *personality);
#endif
...
struct bl_block_meta_s *bl_perftimer_setup (char const *block_name, struct bl_comp_def_s const *comp_def, __attribute__((unused)) void const *personality)
{
    ...
    return bl_default_setup(block_name, comp_def);
}
#endif
...
static void bl_perftimer_start_function(void *ptr, uint32_t period_ns)
{
    ...
    bl_perftimer_block_t *p = (bl_perftimer_block_t *)ptr;
